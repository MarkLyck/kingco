...
class ListingDetails extends React.Component {
  ...
  handleClose = () => {
    ...
    if (
      this.props.history &&
      this.props.history.location.pathname.includes("rentals")
    ) {
      this.props.history.replace("/rentals");
    } else if (
      this.props.history &&
      this.props.history.location.pathname.includes("sales")
    ) {
  ...
  renderDeleteButton = id => {
    if (localStorage.getItem("skAdmin")) {
      return (
        <button className="delete-button" onClick={() => this.handleDelete(id)}>
          Delete Listing
        </button>
      );
    }
        ...
        <Paper className="listing-details-container">
          ...
          <div className="details-content">
            ...
            <div className="button-container">
              <a
                href={`mailto:stephanie@sk-estates.com?Subject=${
                  listing.area
                }%20ref%20${listing.reference}`}
...
const ListingQuery = gql`
  query ListingQuery($ref: String) {
    allListings(filter: { reference: $ref }) {
      id
      reference
      type
      area
